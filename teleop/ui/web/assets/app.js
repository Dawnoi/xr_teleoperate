...
function baseRecordingSummary(recording) {
  const base = recording?.base || {};
  const enabled = !!base.enabled;
  if (!enabled) {
    return `<div class="base-record-status"><span class="pill"><span class="dot"></span>Base recording OFF</span><span class="mini">启动命令未启用 --record-base；本条数据不会写入底盘 state/action。</span></div>`;
  }
  const alive = !!base.receiver_alive;
  return `<div class="base-record-status">
    <span class="pill"><span class="dot ${alive ? "ok" : "err"}"></span>Base recording ON</span>
    <span><b>Receiver</b> ${alive ? "alive" : "offline"}</span>
...
function renderPlayback() {
  ...
  const realReplayBaseSource = String(localStorage.realReplayBaseSource || realReplay.base_source || "none");
...
function startRealReplay() {
  ...
  const baseSourceNode = document.getElementById("realReplayBaseSource");
  ...
  const baseSource = String(baseSourceNode?.value || localStorage.realReplayBaseSource || "none").trim();
  localStorage.realReplaySpeedScale = speedScale;
  localStorage.realReplayArmSource = armSource;
  localStorage.realReplayBaseSource = baseSource;
